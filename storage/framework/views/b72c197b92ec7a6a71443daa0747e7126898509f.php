...
<body>
   ...
   <header class="header" style="text-align:center" > 
    ...
    <?php $ides=""; $desc=""; ?>
    <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php  $ides=$key->IdEstructura; $desc=$key->Descripcion;?> <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>   
...
$result1=$mysqli->query("SELECT LEFT(IdEstructura,4) AS newcod,Descripcion AS denominaci,codigo AS codigo FROM estructura WHERE  LENGTH(NewCodigo)='4' ORDER BY 1 limit 25");
  ...
  $result2=$mysqli->query("SELECT LEFT(IdEstructura,7) AS newcod,Descripcion AS denominaci,codigo AS codigo,LEFT(IdEstructura,7) AS pr FROM estructura WHERE  LENGTH(NewCodigo)=7  AND IdEstructura LIKE '$rs[newcod]%' ORDER BY 1");
    ...
    $i=0;
     while($rs2 =$result2->fetch_assoc()){$i++;   
       ...
       $html.='<tr><td><b>'.$rs2["newcod"].'</b></td><td style="width:1px"></td><td colspan="2"><b>'.$rs2["denominaci"].'</td></b></tr>';
         //=================Tercer nivel=================
         
        $j=0;
        while($rs3 =$result3->fetch_assoc()){$j++;  
           $html.='<tr><td>'.$rs3["newcod"].'</td>
